...
export const withIssueToken = async (
  ...
  if (timeInvalidation) {
    ...
    const timeInvalidatorData = await tryGetAccount(() =>
      timeInvalidator.accounts.getTimeInvalidator(connection, timeInvalidatorId, wallet)
    );
  ...
  if (useInvalidation) {
    ...
    const useInvalidatorData = await tryGetAccount(() =>
      useInvalidator.accounts.getUseInvalidator(connection, useInvalidatorId, wallet)
    );
  ...
  if (kind === TokenManagerKind.Managed) {
    ...
    const mintManagerData = await tryGetAccount(() =>
      tokenManager.accounts.getMintManager(connection, mintManagerId, wallet)
    );
    if (!mintManagerData) {
...
export const withClaimToken = async (
  ...
  const [tokenManagerData, claimApproverData] = await Promise.all([
    tokenManager.accounts.getTokenManager(connection, tokenManagerId, wallet),
    tryGetAccount(() =>
      claimApprover.accounts.getClaimApprover(connection, tokenManagerId, wallet)
    ),
  ]);
