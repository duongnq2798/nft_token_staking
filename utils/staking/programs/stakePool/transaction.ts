...
import { findMintManagerId, findTokenManagerAddress, tokenManagerAddressFromMint } from "utils/token-manager/programs/tokenManager/pda";
...
export const withInitStakePool = async (
  ...
  const identifierData = await tryGetAccount(() =>
    getPoolIdentifier(connection, wallet)
  );
...
export const withUnstake = async (
  ...
  const [stakeEntryData, rewardDistributorData] = await Promise.all([
    tryGetAccount(() => getStakeEntry(connection, stakeEntryId, wallet)),
    tryGetAccount(() => getRewardDistributor(connection, rewardDistributorId, wallet)),
  ]);

  const stakePoolData = await getStakePool(connection, params.stakePoolId, wallet);
...
export const withReturnReceiptMint = async (
  ...
  const stakeEntryData = await tryGetAccount(() =>
    getStakeEntry(connection, params.stakeEntryId, wallet)
  );
  if (!stakeEntryData) {
  ...
  const tokenManagerData = await tryGetAccount(() =>
    tokenManager.accounts.getTokenManager(connection, tokenManagerId, wallet as any)
  );
