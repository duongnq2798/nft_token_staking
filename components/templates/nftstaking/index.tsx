...
import { LoadingSpinner } from 'utils/namespaces-components/common/LoadingSpinner'
...
import ListStakeCard from 'components/organisms/ListStakeCard'
...
const NFTStaking = () => {
  ...
  const [currentTkAddress, setCurrentTkAddress] = useState('')
  ...
  async function handleClaimRewards(token: any) {
    setCurrentTkAddress(token.metadata.pubkey.toString())
    const stakedSelected = [token]
  ...
  async function handleStake(token: any) {
    setCurrentTkAddress(token.metadata.pubkey.toString())
    let unstakedSelected = [token]
    if (!wallet.connected) {
  ...
  async function handleUnstake(token: any) {
    setCurrentTkAddress(token.metadata.pubkey.toString())
    const stakedSelected = [token]
    if (!wallet.connected) {
      notify({ message: `Wallet not connected`, type: 'error' })
  ...
  const getNFTs = () => {
    ...
    const result = unstakeNFT.concat(stakedNFT)
    console.log(result)
    return result
  }
  ...
  return (
    <>
      <CustomHead key={'Nft staking'} title="Token staking" />
      <Container>
        <Header />
        <Stats
